...
public class HbnProductDAO extends AbstractHbnDAO<Product> implements ProductDAO {
    ...
    public ArrayList<Product> getProducts(int page, int limit) {
        ...
        Session ss = getSession();
     //   ss.beginTransaction();
        //Query query = ;//"SELECT * FROM Product p ORDER BY p.price ASC");
//query.setFirstResult(page * limit).setMaxResults(limit).list();
        result = (ArrayList<Product>) ss.createCriteria(Product.class).addOrder(Order.asc("price")).setFirstResult(page * limit)
